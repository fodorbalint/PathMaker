...
namespace SvgApp
{
	internal class Path
	{
			...
			else
			{
				...
				for (i = 0; i < 4; i++)
				{
					...
					if (x - x0 == dx && y - y0 == dy)
					{
						...
						T("possible.Count before CheckNearBorder " + possible.Count);
						CheckNearBorder();	
						Check1x3();
						Check3x3();
						CShape = false;
		...
		{
			T("CheckNearFieldCommon " + InTaken(x + 2 * lx, y + 2 * ly) + " " + InTakenF(leftField));
			if (InTaken(x + 2 * lx, y + 2 * ly) && !InTakenF(leftField)) //2 to left
			{
				...
				if (InTaken(x + lx - dx, y + ly - dy)) //C shape that needs to be filled, unless there is a live end nearby that can fill it
				{
					...
					if (!(liveEndX == leftField[0] && Math.Abs(liveEndY - leftField[1]) == 1 || liveEndY == leftField[1] && Math.Abs(liveEndX - leftField[0]) == 1))
					{
						...
						if (!(path2 != null && leftField[0] == path2[path2.Count - 1][0] && leftField[1] == path2[path2.Count - 1][1]))
						{
							T("C shape left");
							forbidden.Add(straightField);
							forbidden.Add(rightField);
			...
			if (InTaken(x + 2 * rx, y + 2 * ry) && !InTakenF(rightField)) //2 to right
			{
				if (InTaken(x + rx - dx, y + ry - dy))
				{
					...
					{
						if (!(path2 != null && rightField[0] == path2[path2.Count - 1][0] && rightField[1] == path2[path2.Count - 1][1]))
						{
							T("C shape right");
							forbidden.Add(straightField);
							forbidden.Add(leftField);
