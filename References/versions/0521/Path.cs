...
namespace SvgApp
{
	internal class Path
	{
		...
		public void CheckNearFutureSide() //suppose the side is of stair form
		{
			...
			else if (InFuture(x + 2 * dx, y + 2 * dy) && InFuture(x + lx + 2 * dx, y + ly + 2 * dy) && InFuture(x + lx + dx, y + ly + dy) && !InTakenF(leftField) && !InTakenF(straightField) && !InTakenF(rightField) && !InFutureF(leftField) && !InFutureF(straightField) && !InFutureF(rightField))
			{
		...
		{
			//Due to previous extension, a future line may run at 2 distance to the left or right. If it is not the section start, we must step there.
			if (InFuture(x + 2 * lx, y + 2 * ly) && !InFutureStart(x + 2 * lx, y + 2 * ly) && !InTakenF(leftField) && !InFutureF(leftField))
			{
				T("CheckLeftRightFuture to left");
				forbidden.Add(rightField);
				forbidden.Add(straightField);
			}
			else if (InFuture(x + 2 * rx, y + 2 * ry) && !InFutureStart(x + 2 * rx, y + 2 * ry) && !InTakenF(rightField) && !InFutureF(rightField))
			{
		...
		public bool InFutureStart(int x, int y)
		{
			...
			foreach (int[] section in MainWindow.futureSections)
			{
				...
				{
					//selectedSection = i;
					foundFutureStartIndex = foundIndex;
					return true;
				}
			}
		...
		public bool InFutureEnd(int x, int y)
		{
			...
			foreach (int[] section in MainWindow.futureSections)
			{
				...
				{
					foundFutureEndIndex = foundIndex;
					return true;
				}
			}
