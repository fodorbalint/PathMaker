...
namespace SvgApp
{
	internal class Path
	{
		...
		public void NextStepPossibilities(bool searchReverse = true, int startIndex = -1, int nearSection = -1, int farSection = -1)
		{
			nearField = false;
		...
		public bool InFutureOwnStartF(int[] f, int nearSection)
		{
			...
			{
				i++;
				if (i != nearSection && section[0] == foundIndex)
				{
					// nearSection parameter is only added at start when examining possibilities
					if (nearSection != -1)
					{
						foreach (int[] merge in MainWindow.futureSectionMerges)
						{
							// All near ends from the current merge cannot be stepped on. 
		...
		public bool InFutureOwnEndF(int[] f, int farSection)
		{
			int c = path.Count;
			if (c == 0) return false;
			...
			{
				i++;
				if (i != farSection && section[1] == foundIndex)
				{
					// farSection parameter is only added at start when examining possibilities
					if (farSection != -1)
					{
						foreach (int[] merge in MainWindow.futureSectionMerges)
						{
							// All far ends from the current merge cannot be stepped on. 
							if (merge[merge.Length - 1] == farSection)
							{
								for (int j = 1; j < merge.Length; j++)
								{
									if (merge[j] == i) return false;
