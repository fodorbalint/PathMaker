...
namespace SvgApp
{
	internal class Path
	{
		...
		public void CheckLeftRightFuture()
		{
			...
			if (InFuture(x + 2 * lx, y + 2 * ly) && !InFutureStart(x + 2 * lx, y + 2 * ly) && !InFutureStart(x + lx - dx, y + ly - dy) && !InTakenF(leftField) && !InFutureF(leftField))
			{
				T("CheckLeftRightFuture to left");
				forbidden.Add(rightField);
				forbidden.Add(straightField);
			}
			else if (InFuture(x + 2 * rx, y + 2 * ry) && !InFutureStart(x + 2 * rx, y + 2 * ry) && !InFutureStart(x + rx - dx, y + ry - dy) && !InTakenF(rightField) && !InFutureF(rightField))
			{
