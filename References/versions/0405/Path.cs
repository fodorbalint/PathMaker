...
namespace SvgApp
{
	internal class Path
	{
		...
		public int foundFutureStartIndex = -1;
		public int foundFutureEndIndex = -1;
			...
			else
			{
				...
				for (i = 0; i < 4; i++)
				{
					...
					if (x - x0 == dx && y - y0 == dy)
					{
						...
						if (!InTakenF(straightField) && !InBorderF(straightField))
						{
							T("possible straight");
							possible.Add(straightField);
						}
						if (!InTakenF(rightField) && !InBorderF(rightField))
						{
							T("possible right");
							possible.Add(rightField);
						}
						if (!InTakenF(leftField) && !InBorderF(leftField))
						{
							T("possible left");
		...
		public void CheckNearBorder()
		{
			...
			else if (leftField[directionIndex] == size && !InTakenF(leftField))
			{
		...
		{
			bool CShape = false; //left/right across checking will be disabled, no exits needed
			// o
			//xo
			//xox
			...
			if (InTaken(x + 2 * lx, y + 2 * ly) && !InTakenF(leftField)) //2 to left
			{
			...
			if (InTaken(x + 2 * rx, y + 2 * ry) && !InTakenF(rightField)) //2 to right
			{
			...
			if (InTaken(x + 2 * lx + dx, y + 2 * ly + dy) && !InTakenF(leftField) && !InTaken(x + lx + dx, y + ly + dy))
			{
			...
			if (InTaken(x + 2 * rx + dx, y + 2 * ry + dy) && !InTakenF(rightField) && !InTaken(x + rx + dx, y + ry + dy))
			{
		...
		{
			if (InFuture(x + 2 * dx, y + 2 * dy) && InFuture(x + rx + 2 * dx, y + ry + 2 * dy) && InFuture(x + rx + dx, y + ry + dy) && !InTakenF(leftField) && !InTakenF(straightField) && !InTakenF(rightField) && !InFutureF(leftField) && !InFutureF(straightField) && !InFutureF(rightField))
			{
			...
			if (InFuture(x + 2 * dx, y + 2 * dy) && InFuture(x + lx + 2 * dx, y + ly + 2 * dy) && InFuture(x + lx + dx, y + ly + dy) && !InTakenF(leftField) && !InTakenF(straightField) && !InTakenF(rightField) && !InFutureF(leftField) && !InFutureF(straightField) && !InFutureF(rightField))
			{
				T("CheckNearFutureSide mid left");
				forbidden.Add(leftField);
				forbidden.Add(straightField);
			}
			else if (InFuture(x + rx + 2 * dx, y + ry + 2 * dy) && InFuture(x + 2 * rx + 2 * dx, y + 2 * ry + 2 * dy) && InFuture(x + 2 * rx + dx, y + 2 * ry + dy) && !InTakenF(leftField) && !InTakenF(straightField) && !InTakenF(rightField) && !InFutureF(leftField) && !InFutureF(straightField) && !InFutureF(rightField))
			{
				//will also be true near the far end if we are approaching the near end
				T("CheckNearFutureSide across right");
				//determine future line direction
				int i1 = InFutureIndex(x + rx + 2 * dx, y + ry + 2 * dy);
				int i2 = InFutureIndex(x + 2 * rx + dx, y + 2 * ry + dy);

				if (i1 > i2) //we will have to connect to the side in front of us. The right area cannot be filled, because we cannot get out of it
				{
					//can 2 rx and rx -dx fields be filled at this point? Then we can only go to right
					forbidden.Add(leftField);
					forbidden.Add(rightField);
					forbidden.Add(straightField);
				}
				else //Connecting to right side. We have to fill the area to the left. Can only go to left.
				{
					forbidden.Add(rightField);
					forbidden.Add(straightField);
				}
			}
			else if (InFuture(x + lx + 2 * dx, y + ly + 2 * dy) && InFuture(x + 2 * lx + 2 * dx, y + 2 * ly + 2 * dy) && InFuture(x + 2 * lx + dx, y + 2 * ly + dy) && !InTakenF(leftField) && !InTakenF(straightField) && !InTakenF(rightField) && !InFutureF(leftField) && !InFutureF(straightField) && !InFutureF(rightField))
			{
				T("CheckNearFutureSide across left");
				//determine future line direction
				int i1 = InFutureIndex(x + lx + 2 * dx, y + ly + 2 * dy);
				int i2 = InFutureIndex(x + 2 * lx + dx, y + 2 * ly + dy);

				if (i1 > i2) //we will have to connect to the side in front of us. The left area cannot be filled, because we cannot get out of it
				{
					//can 2 lx and lx -dx fields be filled at this point? Then we can only go to left
					forbidden.Add(leftField);
					forbidden.Add(rightField);
					forbidden.Add(straightField);
				}
				else //Connecting to left side. We have to fill the area to the right. Can only go to right.
				{
					forbidden.Add(leftField);
					forbidden.Add(straightField);
				}
		...
		public bool InTaken(int x, int y) //more recent fields are more probable to encounter, so this way processing time is optimized
		{
			if (!isMain)
			{
				...
				if (!searchReverse)
				{
					for (int i = searchStartIndex + 2; i < c1; i++) //the first area that is checked (in checknearfield) is one back and one to left/right side, 2 steps from the current
					{
						int[] field = path[i];
						if (x == field[0] && y == field[1])
						{
		...
		public bool InFutureStart(int x, int y)
		{
			...
			//T("InFutureStart prevX " + prevX + " prevY " + prevY);

			if (x == prevX && y == prevY)
			{
				foundFutureStartIndex = c2 - 1;
				//T("Return at start");
				return true;
			}
			...
			for (int i = c2 - 2; i >= 0; i--)
			{
				...
				if (x == searchX && y == searchY && !(searchX == prevX && Math.Abs(searchY - prevY) == 1 || searchY == prevY && Math.Abs(searchX - prevX) == 1))
				{
					//T("Return at middle " + i);
					/*foreach (int[] field2 in path2)
					{
						T("Future x " + field2[0] + " y " + field2[1]);
					}*/
					foundFutureStartIndex = i;
					return true;
				}
				prevX = searchX;
				prevY = searchY;
		...
		{			
			if (foundFutureStartIndex == -1) return false;

			int c2 = path2.Count;
			if (c2 == 0) return false;

			int i = foundFutureStartIndex;
			int[] field = path2[i];
			int searchX = field[0];
			int searchY = field[1];

			//T("InFutureend " + searchX + " " + searchY + " " + foundFutureStartIndex + " " + x + " " + y);
			int prevX;
			int prevY;
			...
			do {
				...
				i--;
			} while (i > -1 && (searchX == prevX && Math.Abs(searchY - prevY) == 1 || searchY == prevY && Math.Abs(searchX - prevX) == 1));

			if (i == -1) //last future field
			{
				if (path2[i + 1][0] == x && path2[i + 1][1] == y)
				{
					foundFutureEndIndex = i + 1;
					return true;
				}
			}
			else //jumped to new section
			{
				if (path2[i + 2][0] == x && path2[i + 2][1] == y)
				{
					foundFutureEndIndex = i + 2;
					return true;
				}
			}
			return false;
